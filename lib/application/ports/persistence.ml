...
type container_metadata = { attempt_id : Attempt_id.t; worker_id : Worker_id.t;
  ...
  removed_at : Timestamp.t option; cleanup_outcome : cleanup_outcome }
...
type control_repository = {
  ...
  create_stop_unknown : worker_id:Worker_id.t -> attempt_id:Attempt_id.t -> requested_at:Timestamp.t -> (bool, error) result;
  confirm_stop_unknown : worker_id:Worker_id.t -> attempt_id:Attempt_id.t -> completed_at:Timestamp.t -> (unit, error) result;
}
...
type repositories = { jobs : job_repository; attempts : attempt_repository;
  workers : worker_repository; events : event_repository; controls : control_repository;
  logs : log_repository; containers : container_repository; metrics : metrics_repository }
type t = { with_transaction : 'a 'e.
  (repositories -> ('a, 'e) result) -> (('a, 'e) result, error) result }
