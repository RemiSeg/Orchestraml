...
let create ~id ~job_id ~number ~worker_id ~assigned_at =
  ({ id; job_id; number; worker_id; status = Attempt_status.Assigned; assigned_at;
    started_at = None; finished_at = None; outcome = None } : t)
...
let snapshot (value : t) = { id = value.id; job_id = value.job_id; number = value.number;
  ...
  started_at = value.started_at; finished_at = value.finished_at; outcome = value.outcome }
...
let restore snapshot =
  let value : t = { id = snapshot.id; job_id = snapshot.job_id; number = snapshot.number;
    ...
    started_at = snapshot.started_at; finished_at = snapshot.finished_at; outcome = snapshot.outcome } in
  let chronological =
    (match value.started_at with None -> true | Some started -> Timestamp.compare started value.assigned_at >= 0)
  ...
  if not chronological then Error (Validation_error.make ~field:"attempt_timestamps" "are not chronological")
    ...
    | Attempt_status.Running when value.started_at <> None && value.finished_at = None && value.outcome = None -> Ok value
    ...
    | Attempt_status.Failed | Attempt_status.Lost | Attempt_status.Cancelled
      when value.finished_at <> None && outcome_matches value.status value.outcome -> Ok value
